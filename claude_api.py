...
def _build_batch_prompt(body: BatchQuestionRequest) -> str:
...
def _normalize_question_items(items: list, body: BatchQuestionRequest) -> list[dict]:
    if not items:
...
async def _generate_questions_batch(body: BatchQuestionRequest) -> list[dict]:
    prompt = _build_batch_prompt(body)
    ...
    for attempt in range(2):
        try:
            ...
            return _normalize_question_items(items, body)
        except (KeyError, ValueError, json.JSONDecodeError) as exc:
            last_error = exc
            max_tokens = min(8192, max_tokens + 2000)
            prompt = (
                _build_batch_prompt(body)
                + "\n\nYour previous response was invalid or truncated. "
                "Return compact valid JSON with shorter solution fields."
            )

    # Fall back to one question per call — slower but avoids truncated batch JSON.
    ...
    for _ in range(body.count):
        ...
        response = await client.messages.create(
            model=MODEL,
            max_tokens=1200,
            messages=[{"role": "user", "content": _build_batch_prompt(single)}],
        )
        try:
            items = parse_json_array(response.content[0].text)
            normalized = _normalize_question_items(items, single)
        except (KeyError, ValueError, json.JSONDecodeError) as exc:
            raise HTTPException(
                status_code=502,
...
async def generate_questions_batch(body: BatchQuestionRequest):
    try:
        return await _generate_questions_batch(body)
    except APIError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
